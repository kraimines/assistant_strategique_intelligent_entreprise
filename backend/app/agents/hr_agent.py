"""HR domain agent node.

Implements a tool-calling ReAct loop (up to MAX_TOOL_ITERATIONS rounds) that
uses the six HR tools to answer the user's question, then stores the final AI
...
from app.prompts.hr_prompts import HR_SYSTEM_PROMPT
...
def _build_tools_by_name() -> Dict[str, Any]:
    ...
    """
    return {tool.name: tool for tool in HR_TOOLS}


# ── Node ──────────────────────────────────────────────────────────────────────

def hr_agent_node(state: AgentState) -> AgentState:
    """HR domain agent: answers HR questions via a bounded tool-calling loop.

    Workflow
    --------
    1. Build ``llm_with_tools`` by binding all HR tools to the base LLM.
    2. Prepend a SystemMessage containing HR_SYSTEM_PROMPT to the conversation.
    ...
    # ── Bind tools to LLM ────────────────────────────────────────────────────
    try:
        llm_with_tools = get_llm_with_tools(HR_TOOLS)
    except Exception as exc:
        logger.exception("hr_agent_node: failed to bind tools to LLM — %s", exc)
        return {
            **state,
            "error_message": f"Erreur d'initialisation de l'agent RH : {exc}",
        }
    ...
    for iteration in range(1, MAX_TOOL_ITERATIONS + 1):
        logger.debug("hr_agent_node: LLM invoke iteration %d", iteration)
        ...
        try:
            ...
            logger.debug(
                "hr_agent_node: LLM iteration %d completed in %.1f ms", iteration, llm_ms
            )
        except Exception as exc:
            logger.exception(
                "hr_agent_node: LLM invocation failed at iteration %d — %s",
                iteration,
                exc,
            )
            error_msg = f"Erreur lors de la génération de la réponse RH : {exc}"
            return {
                **state,
        ...
        if not tool_calls:
            ...
            logger.info(
                "hr_agent_node: no tool calls at iteration %d — treating as final answer",
                iteration,
            )
        ...
        for tool_call in tool_calls:
            ...
            logger.info("hr_agent_node: calling tool '%s' args=%s", tool_name, tool_args)
            t_tool = time.perf_counter()

            if tool_name not in tools_by_name:
                tool_result: Any = {"error": f"Outil inconnu : '{tool_name}'"}
                logger.warning("hr_agent_node: unknown tool requested — '%s'", tool_name)
            else:
                try:
                    tool_result = tools_by_name[tool_name].invoke(tool_args)
                except Exception as exc:
                    logger.exception(
                        "hr_agent_node: tool '%s' raised an exception — %s",
                        tool_name,
                        exc,
                    )
            ...
            logger.info(
                "hr_agent_node: tool '%s' completed in %.1f ms", tool_name, tool_ms
            )
        ...
        if requires_report and accumulated_tool_results:
            logger.info(
                "hr_agent_node: report mode — skipping synthesis LLM call, "
                "passing raw tool results to final_response_node"
            )
            new_messages.append(ai_response)
            break

        # If this was the last allowed iteration, force a final LLM call without tools
        if iteration == MAX_TOOL_ITERATIONS:
            logger.warning(
                "hr_agent_node: reached MAX_TOOL_ITERATIONS=%d — forcing final response",
                MAX_TOOL_ITERATIONS,
            )
            try:
                ...
                logger.debug(
                    "hr_agent_node: final forced LLM call completed in %.1f ms", final_ms
                )
                new_messages.append(final_response)
            except Exception as exc:
                logger.exception(
                    "hr_agent_node: final forced LLM call failed — %s", exc
                )
                return {
                    **state,
                    "messages": new_messages,
    ...
    logger.info(
        "hr_agent_node done — %d new messages, %d tool results — %.1f ms total",
        len(new_messages),
